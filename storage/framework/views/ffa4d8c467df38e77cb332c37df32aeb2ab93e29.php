...
<?php $__env->startSection('content'); ?>
    <div class="col-sm-9 padding-right">
        <div class="features_items"><!--features_items-->
            ...
            <h2 class="title text-center"><?php echo e($objCat->name); ?></h2>
            <div class="row">
                <div class="col-md-3">
                    <?php
                    if ($objCat->parrent_cat == '') {
                        $catId = $objCat->id;
                    } else {
                        $catId = $objCat->parrent_cat;
                    }
                    ?>
                    <select id="cat_filter" class="filter form-control">
                        <option value="<?php echo e(null); ?>">-- Chọn danh mục --</option>
                        <option <?php if($objCat->parrent_cat == ''): ?> selected <?php endif; ?> value="all">Tất cả danh mục</option>
                        <?php $__currentLoopData = $objSubCat1[$catId]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $subCat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option <?php if($objCat->id == $subCat->id): ?> selected
                                    <?php endif; ?> value="<?php echo e($subCat->id); ?>"><?php echo e($subCat->name); ?></option>
            ...
            <div id="list-product">
                <?php if(sizeof($objProduct) == 0): ?>
                    <p class="alert alert-warning text-center"> Chưa có sản phẩm nào thuộc danh mục này được đăng
                        bán </p>
                <?php else: ?>
                                ...
                                <div class="choose row">
                                    <div class="col-xs-6 pull-left">
                                        <a style="color: #b3afa8; font-size: 13px;"
                                           href="<?php echo e(route('subshop.index', ['slug' => str_slug($product->shop_name), 'id' => $product->shop_id])); ?>">
                                    ...
                                    <div class="col-xs-6 pull-right">
                                        <a style="color: #b3afa8; font-size: 13px;"
                                           onclick="changeActive(<?php echo e($product->id); ?>)" href="javascript:void(0)">
...
<?php $__env->startSection('js'); ?>
    <script>
        $(document).on('change', '.filter', function () {
            $('#spin').show();
            var current_cat = '<?php echo e($objCat->id); ?>';
        ...
        function getProduct(current_cat, cat_filter, price_filter, status_filter, promotion_filter, page, order_filter) {
            $('#spin').show();
            data = {
                current_cat: current_cat,
                cat_filter: cat_filter,
